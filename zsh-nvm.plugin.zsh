...
_zsh_nvm_latest_release_tag() {
  echo $(builtin cd "$NVM_DIR" > /dev/null && git fetch --quiet --tags origin && git describe --abbrev=0 --tags --match "v[0-9]*" $(git rev-list --tags --max-count=1))
}

_zsh_nvm_install() {
  echo "Installing nvm..."
  git clone https://github.com/nvm-sh/nvm.git "$NVM_DIR"
  $(builtin cd "$NVM_DIR"  > /dev/null && git checkout --quiet "$(_zsh_nvm_latest_release_tag)")
}
...
_zsh_nvm_upgrade() {
  ...
  local installed_version=$(builtin cd "$NVM_DIR" > /dev/null && git describe --tags)
  ...
  if [[ "$installed_version" = "$latest_version" ]]; then
    ...
    $(builtin cd "$NVM_DIR" > /dev/null && git fetch --quiet && git checkout "$latest_version")
    _zsh_nvm_load
  fi
}
...
_zsh_nvm_revert() {
  ...
  if [[ -n "$previous_version" ]]; then
    ...
    $(builtin cd "$NVM_DIR" > /dev/null && git checkout "$previous_version")
    _zsh_nvm_load
  else
